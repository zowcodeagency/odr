...
files.forEach((f, i) => {
  ...
  // .html/.json files are real TS-resolvable modules (HTMLBundle / parsed JSON) even
  // under `type: "file"`, so cast to the string path `with { type: "file" }` actually gives at runtime.
  entries.push(`  ${JSON.stringify(url)}: a${i} as unknown as string,`);
});
...
const migFiles = walk(drizzleDir).filter((f) => !f.includes("_snapshot.json"));
const migEntries: string[] = [];
migFiles.forEach((f, i) => {
  const rel = relative(drizzleDir, f).split("\\").join("/");
  lines.push(`import m${i} from ${JSON.stringify("../../../packages/db/drizzle/" + relative(drizzleDir, f))} with { type: "file" };`);
...
// Bun only accepts --windows-hide-console when compiling *on* Windows itself,
