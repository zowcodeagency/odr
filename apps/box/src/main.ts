...
import { phoneUrls, startBox } from "./serve.ts";
...
try {
  ...
  const lan = phoneUrls(new URL(box.url).port);
  console.log(`Odr Box · data in ${dataDir}`);
  console.log(`On this computer: ${box.url}`);
  console.log(lan.length ? `On a phone on this wifi: ${lan.join("  or  ")}` : "No wifi/ethernet address found — connect this computer to the restaurant wifi.");
